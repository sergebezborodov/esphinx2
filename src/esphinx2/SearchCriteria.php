...
class SearchCriteria extends Component
{
     ...
     */
    public $matchMode = enum\Match::ALL;
     ...
     * @var int
     */
    public $groupByFunc = enum\Group::BY_ATTR;
    ...
    public function addFilters($filters)
    {
        if (empty($filters) || !is_array($filters)) {
            throw new Exception('Filters must be a non empty array');
        }
    ...
    {
        if ($this->sortMode == enum\Sort::EXTENDED) {
            throw new Exception('Use getSortBy is not allowen with EXTENDED sort mode, use getOrders');
        }
    ...
    {
        if ($this->sortMode == enum\Sort::EXTENDED) {
            throw new Exception('Use setSortBy is not allowen with EXTENDED sort mode, use getOrders');
        }
    ...
    {
        if ($this->sortMode != enum\Sort::EXTENDED) {
            throw new Exception('addOrder method can uses only with EXTENDED sort mode, '
    ...
    public function addOrder($attribute, $order)
    {
        ...
        if ($order != 'ASC' && $order != 'DESC') {
            throw new Exception('Invalid value for sorting direction');
        }
    ...
    public function addOrders($orders)
    {
        ...
        if (empty($orders) || !is_array($orders)) {
            throw new Exception('Orders must be a non empty array');
        }
    ...
    public function addFieldWeight($field, $weight)
    {
        if (!is_integer($weight)) {
            throw new Exception('Field weight must be integer');
        }
    ...
    public function addFieldWeights($weights)
    {
        if (empty($weights) || !is_array($weights)) {
            throw new Exception('Weights must be a non empty array');
        }
    ...
    {
        if (!is_integer($weight)) {
            throw new Exception('Index weight must be integer');
        }
    ...
    public function setSortMethod($sortMethod)
    {
        if ($sortMethod != 'pq' && $sortMethod != 'kbuffer') {
            throw new Exception('Invalid value for sortMethod');
        }
    ...
    public function setIdf($idf)
    {
        if ($idf != 'normalized' && $idf != 'plain') {
            throw new Exception('Invalid value for idf flag');
        }
